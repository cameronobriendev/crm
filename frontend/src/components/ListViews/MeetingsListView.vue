...
// A meeting has no page of its own. It is read on the lead it was filed
// against, on the Meetings tab, which is what the hash selects. A meeting
// filed against anything else has nowhere to go, so its row does not travel.
function getRowRoute(row) {
  if (row.reference_doctype !== 'CRM Lead' || !row.reference_docname)
    return null
